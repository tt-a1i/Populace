...
async def create_save(payload: SaveRequest, request: Request) -> dict[str, Any]:
    ...
    return _save_meta(save_id, data)
...
async def list_saves() -> list[dict[str, Any]]:
    ...
    for path in sorted(SAVES_DIR.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True):
        try:
            with open(path, encoding="utf-8") as fh:
                data = json.load(fh)
            saves.append(_save_meta(path.stem, data))
        except Exception:
            pass  # Skip corrupt files
    return saves


@router.post("/{save_id}/load")
async def load_save(save_id: str, request: Request) -> dict[str, Any]:
